...
def extract_orgName_from_file(xml_file):
    ...
    try:
        ...
        # Extract all orgName elements and their context

        org_data = []

        for orgName in root.findall('.//{http://www.tei-c.org/ns/1.0}orgName'):

            # Get the text content and the 'n' attribute of the orgName element

            name_text = orgName.text

            n_attr = orgName.get('n')
            ...
            if n_attr:
                ...
                if n_pos != -1:
                    ...
                    if name_text:
                        ...
                        org_data.append((os.path.abspath(xml_file), name_text,document_date ,n_attr, context))

            else:

                # If 'n' attribute is not present, extract context around the orgName element

                start_pos = xml_content.find(ET.tostring(orgName, encoding='unicode'))

                end_pos = start_pos + len(ET.tostring(orgName, encoding='unicode'))
                ...
                if name_text:
                    ...
                    org_data.append((os.path.abspath(xml_file), name_text, document_date,None, context))
...
def extract_orgName_from_folder(folder_path):
        ...
        for filename in files:
            ...
            if filename.endswith(".xml"):
                ...
                try:
                    ...
                    org_data = extract_orgName_from_file(file_path)
    ...
    df = pd.DataFrame(all_org_data, columns=['FilePath', 'orgName', 'documentDate','n', 'Context'])

    return df

def main(input_folder, output_csv):     
    df = extract_orgName_from_folder(input_folder)    
    df.to_csv(output_csv, index=False)     
    print(f"Data has been saved to {output_csv}") 
    
if __name__ == "__main__":     
    parser = argparse.ArgumentParser(description="Extract orgName elements from TEI-XML files.")     
